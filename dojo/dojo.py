import os
import random
from room.office_space import OfficeSpace
...
functionality needed in the system"""
    ...
    def print_allocations(self, file_name=None):
        ...
        if file_name is not None:  # no file name to write to. just print
            if len(file_name) < 4:
                ...
                return file_name + ":" + string_to_print
        else:
            return "none:" + string_to_print
    ...
    def load_people(self, file_name):
        ...
        if len(file_name) < 4:
            ...
            print("The file entered doesn't exist")
        elif not os.path.getsize(file_name) > 0:
            print("The file entered is empty")
        else:
            try:
                with open(file_name, "r") as file_to_read:
                    ...
                    for line in data:
                        words = line.split()
                        if not len(words):  # the line is empty.. do nothing
                            continue
                        elif len(words) == 4:  # the command has 4 args
                            self.add_person(words[0], words[1], words[2], words[3])
                        elif len(words) == 3:  # the command has only 3 args, hence no accommodation
